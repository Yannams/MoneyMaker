...
CREATE INDEX IF NOT EXISTS idx_payment_links_target_client_id ON public.payment_links(target_client_id);

DROP FUNCTION IF EXISTS public.get_payment_link_details(TEXT);
...
AS $$
  SELECT
    ...
    c.email AS linked_client_email,
    (
      pl.active
  ...
  LEFT JOIN public.clients c ON c.id = pl.target_client_id
...
BEGIN
  ...
  IF v_link.target_client_id IS NOT NULL THEN
    SELECT c.id, c.name, c.phone, c.email
    INTO v_client_id, v_customer_name, v_customer_phone, v_customer_email
    FROM public.clients c
    WHERE c.id = v_link.target_client_id;

    IF v_client_id IS NULL THEN
      RAISE EXCEPTION 'Client du lien introuvable';
    END IF;
  ELSE
...
REVOKE ALL ON FUNCTION public.create_personal_payment_link(UUID, UUID, TIMESTAMPTZ, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_personal_payment_link(UUID, UUID, TIMESTAMPTZ, INTEGER) TO authenticated;
